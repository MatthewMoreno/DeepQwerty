...
public class RookModel : PieceModel
{
	...
	public static RookModel GetRookModel(bool color, int[] loc, ChessSquareModel squ, ChessBoardModel b)
	{
		RookModel res = ScriptableObject.CreateInstance<RookModel>();
		res.white = color;
		res.location = loc;
		res.habitat = squ;
		res.theBoard = b;
		int[] spot = new int[2];
		res.moveStructure = new int[28][];
		for(int i = 0; i < 7; i++)
		{
			spot[0] = 0;
			spot[1] = i + 1;
	...
	{
		return GetRookModel(white, location, habitat, theBoard);
	}
